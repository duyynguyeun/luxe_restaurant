...
function useIsFirstMount() {
  const isFirstMount = (0, import_react2.useRef)(true);
...
function useStableValue(value) {
  const [stableValue, setStableValue] = (0, import_react3.useState)(() => value);
...
function useLocalStorageHook(key, initialValue, options) {
  ...
  const deserializer = (0, import_react14.useMemo)(
    () => options?.raw ? (value) => value : options?.deserializer ?? JSON.parse,
    [options]
  );
  const serializer = (0, import_react14.useMemo)(
    () => options?.raw ? String : options?.serializer ?? JSON.stringify,
    [options]
  );
  const initializer = (0, import_react14.useRef)((k) => {
    try {
  ...
  const [state, setState] = (0, import_react14.useState)(() => initializer.current(key));
  (0, import_react14.useLayoutEffect)(() => setState(initializer.current(key)), [key]);
  const set = (0, import_react14.useCallback)(
    (patch) => {
      try {
...
function useSetState(initialState = {}) {
  const [state, set] = (0, import_react15.useState)(initialState);
  const setState = (0, import_react15.useCallback)((patch) => {
    set((previousState) => ({
      ...previousState,
...
function useMeasure(target, debounce = 0) {
  const [element, setElement] = (0, import_react17.useState)(getElement(target));
  const [dimensions, setDimensions] = (0, import_react17.useState)(getElementMeasure(element));
...
function useMediaQuery(input) {
  ...
  const [matches, setMatches] = (0, import_react18.useState)(getMatches(input));
...
function useRenderCount(name) {
  const count = (0, import_react21.useRef)(1);
...
function useResponsiveBase(breakpoints, initialWidth = Infinity, initialHeight = Infinity) {
  ...
  const getScreen = (0, import_react22.useCallback)(() => {
  ...
  const [screen, setScreen] = (0, import_react22.useState)(getScreen());
...
function useScript(src, idOrOptions = {}) {
  ...
  const script = (0, import_react23.useRef)();
  const [state, setState] = (0, import_react23.useState)({
    loaded: false,
    error: false
  });
  const onLoad = (0, import_react23.useCallback)(() => {
    setState({
      loaded: true,
      error: false
    });
  }, []);
  const onError = (0, import_react23.useCallback)(() => {
    if (script.current) {
...
function useSingleton(callback) {
  const hasBeenCalled = (0, import_react24.useRef)(false);
...
function useTimeout(callback, delayMs = 0) {
  const status = (0, import_react25.useRef)("pending");
  const timeout = (0, import_react25.useRef)();
  const savedCallback = (0, import_react25.useRef)(callback);
  const clear = (0, import_react25.useCallback)(() => {
    status.current = "cancelled";
    timeout.current && clearTimeout(timeout.current);
  }, []);
  const set = (0, import_react25.useCallback)(() => {
    status.current = "pending";
    timeout.current && clearTimeout(timeout.current);
    timeout.current = setTimeout(() => {
      status.current = "completed";
  ...
  const getStatus = (0, import_react25.useCallback)(() => status.current, []);
...
function useThrottle(callback, delayMs = 500, trailing = false) {
  const [now, setNow] = (0, import_react26.useState)(0);
  const callbackRef = (0, import_react26.useRef)(callback);
  const hasPendingCall = (0, import_react26.useRef)(false);
...
function useThrottleValue(value, delayMs) {
  ...
  const hasNextValue = (0, import_react27.useRef)(false);
  const nextValue = (0, import_react27.useRef)(null);
  const timer = (0, import_react27.useRef)();
...
function useToggle(initialValue = true) {
  const [value, toggle] = (0, import_react28.useReducer)(
    (state, nextValue) => typeof nextValue === "boolean" ? nextValue : !state,
    initialValue
  );
  const toggleOn = (0, import_react28.useCallback)(() => toggle(true), []);
  const toggleOff = (0, import_react28.useCallback)(() => toggle(false), []);
  return (0, import_react28.useMemo)(() => [value, { toggle, toggleOn, toggleOff }], [value, toggleOn, toggleOff]);
}
...
function useUpdate() {
  const [, update] = (0, import_react29.useReducer)(updateReducer, 0);
  return update;
}
...
function useWhyDidYouUpdate(props, nameOrOptions = {}) {
  const [changes, setChanges] = (0, import_react31.useState)({});
  const previousProps = (0, import_react31.useRef)();
...
function useWindowSize(debounce = 0) {
  ...
  const timeoutRef = (0, import_react32.useRef)(0);
  const handleResize = (0, import_react32.useRef)(() => {
    window.clearTimeout(timeoutRef.current);
    timeoutRef.current = window.setTimeout(() => {
      setSize({
